...
exports.startHistoryCleanup = exports.runPriceHistoryMaintenance = void 0;
const prisma_1 = __importDefault(require("../lib/prisma"));
const runPriceHistoryMaintenance = async () => {
    ...
    try {
        ...
        const totalRecords = await prisma_1.default.priceHistory.count();
        console.log(`📊 Total price records: ${totalRecords}`);
        // Don't run maintenance if DB is too fresh (less than 100 records)
        if (totalRecords < 100) {
            console.log("⚠️  Not enough records to compress yet, skipping maintenance");
            return;
        }
        // STEP 1: Delete everything older than 1 year
        const deleted = await prisma_1.default.priceHistory.deleteMany({
...
exports.runPriceHistoryMaintenance = runPriceHistoryMaintenance;
const compressRecords = async ({ from, to, intervalMinutes, label, }) => {
    const records = await prisma_1.default.priceHistory.findMany({
    ...
    const compressed = Array.from(buckets.values()).map((bucket) => ({
        price: parseFloat((bucket.prices.reduce((a, b) => a + b, 0) / bucket.prices.length).toFixed(4)),
        createdAt: bucket.time,
    }));
    await prisma_1.default.priceHistory.deleteMany({
        where: { createdAt: { gte: from, lt: to } },
    });
    await prisma_1.default.priceHistory.createMany({ data: compressed });
    console.log(`✅ ${label}: ${records.length} records → ${compressed.length} averages`);
};
const startHistoryCleanup = () => {
    ...
    setInterval(() => {
        (0, exports.runPriceHistoryMaintenance)();
    }, 24 * 60 * 60 * 1000);
};
exports.startHistoryCleanup = startHistoryCleanup;
